...
print.gbp <- function(x, ...) {
  ...
  if (!identical(x$x, NA)){
    ...
    if (x$model == "gr") {
      temp <- data.frame(sample.mean = x$sample.mean, se = x$se, x = cova, prior.mean = x$prior.mean,
      ...
      temp <- data.frame(sample.mean = x$sample.mean, n = x$se, x = cova, prior.mean = x$prior.mean,
                         shrinkage = x$shrinkage, sd.shrinkage = x$sd.shrinkage, 
                         post.intv.low = x$post.intv.low, post.mean = x$post.mean, 
                         post.intv.upp = x$post.intv.upp, post.sd = x$post.sd)
    }
...
summary.gbp <- function(object, ...) {
  ...
  if (length(n) %% 2) {
    summary.index <- (n == min(n) | n == max(n) | n == sort(n)[(length(n) + 1) / 2])
    summary.table <- temp[summary.index, ]
    number.of.medians <- dim(summary.table)[1] - 2
    row.names(summary.table) <- c("Obs w/ min(n)", 
                                  paste("Obs w/ median(n)", 1 : number.of.medians, sep = ""),
                                  "Obs w/ max(n)")
  } else { 
    summary.index <- (n == min(n) | n == max(n) | n == sort(n)[length(n) / 2] |
                      n == sort(n)[length(n) / 2 + 1])
    summary.table <- temp[summary.index, ]
    number.of.medians <- dim(summary.table)[1] - 2
    row.names(summary.table) <- c("Obs w/ min(n)", 
                                  paste("Obs w/ median(n)", 1 : number.of.medians, sep = ""),
