...
where $\mu_{0j} =x_{j}'\beta,~j=1, \ldots, 8$, and $x_{j}$ is $j$-th school's covariate vector ($m\times 1$). Also, $m$ is the number of regression coefficients, which is just 1 for an intercept term if there is no covariate. For reference, a paramter with a zero subscript represents a mean parameter of the prior (second-level) distribution. Also,  Based on this conjugate prior distribution, it is easy to derive corresponding posterior distribution. 
\begin{equation}
