...
A benefit of \pkg{Rgbp} is that it produces non-random output and so results can easily be reproduced and compared across studies. In addition to being a standalone analysis tool the package can be used as an aid or a tool in a broader estimation procedure. For example, convergence of an MCMC can be increased by using the point estimates from \pkg{Rgbp} to initialize the chain. 

Due to the speed and ease of use \pkg{Rgbp} can be used as a method of preliminary data analysis. The results of which may inform the statistician whether a more intensive (in terms of coding and computational time) but more precise method (such as MCMC) should be used. 
...
to initialize an MCMC to speed up time to convergence. to check the validity of the code of an MCMC program.

as a demo that a particular prior gives adequate coverage. for frequentist procedure checking.
