...
The 95\% interval plot shows the range of true batting average for each player, which clarifies the regression toward the mean (RTTM) within two groups. The 10th, 14th, and 15th players, for example, are outfielders but their observed batting averages are far lower than the first five outfielders. Their observed batting averages close to their lower limits of interval estimates tells us it can happen by their bad luck. RTTM suggests that in the long run their batting averages will shrink towards the expected prior mean as outfielders (blue horizontal lines).
