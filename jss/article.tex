...
\Abstract{
\pkg{Rgbp} is an R package that utilizes Bayesian machinery to provide a method of estimating two-level hierarchical models (also called random-effects mixed models) for Normal, Poisson, and Binomial data in a fast and computationally efficient manner. Good frequency property is another feature of \pkg{Rgbp}, which can be validated via its repeated sampling procedure called ``method checking."  It is found that such Bayesian-frequentist reconciliation allows \pkg{Rgbp} to have strong points of both sides, working well in small samples and yielding good coverage probabilities for its interval estimates.}
...
The hyper-prior distribution is an assumed distribution on the second-level parameters and with the goal of objectivity in mind \code{gbp} assumes non-informative hyper-prior distributions.
\begin{equation}
