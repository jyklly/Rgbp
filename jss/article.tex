...
In Figure \ref{fig:hospitalcoverage} we see three lines; one is a red line connecting Rao-Blackwellized unbiased estimates for 31 hospitals; another is a dashed blue line connecting simple unbiased estimates for those hospitals; a black horizontal line indicating the confidence level, 0.95. 

The estimated overall average coverage rate is 0.950 from both Rao-Blackwellized and simple unbiased estimates. This result shows that the interval estimates for this particular dataset accurately achieves 95\% confidence under repeated sampling. Note that this estimate depends on the given true value of $r$ and the assumption that the model is true.

If we want to check average Rao-Blackwellized unbiased estimates of coverage rate for each hospital, the following code gives us 31 estimates.
\begin{CodeChunk}
\begin{CodeInput}
R> pcv$coverageRB
\end{CodeInput}
...
As for average simple unbiased estimates of coverage rate for each hospital, we can type the code below to obtain 31 estimates. 
\begin{CodeChunk}
\begin{CodeInput}
R> pcv$coverageS
\end{CodeInput}
...
The last thing to mention is the accuracy of Rao-Blackwellized unbiased estimates compared to that of simple one. Taking the first hospital as an example, we can calculate the sample variance of 1,000 Rao-Blackwellized unbiased estimates for the first hospital and that of 1,000 simple ones by typing following code. For reference, \code{raw.resultRB} and \code{raw.resultS} in \code{pcv} contains all the individual simulation results.
\begin{CodeChunk}
\begin{CodeInput}
...
As seen in Figure \ref{fig:schoolcoverage} the desired $95\%$ confidence (horizontal line at 0.95) is achieved (actually, exceeded) for each school in this example. Here, all the coverage estimates depend on the given true values of $A$ and $\beta_{0}$, and the assumption that the model is true.

Average Rao-Blackwellized unbiased estimates of coverage rate for each school can be seen as below.
\begin{CodeChunk}
\begin{CodeInput}
R> gcv$coverageRB
\end{CodeInput}
...
As for average simple unbiased estimates of coverage rate for each school, 
\begin{CodeChunk}
\begin{CodeInput}
R> gcv$coverageS
\end{CodeInput}
...
Finally  when it comes to the first school,  Rao-Blackwellized unbiased estimate is about 24 times more accurate than simple one. It is based on the ratio of the sample variances of individual 1,000 estimates from two approaches.
\begin{CodeChunk}
\begin{CodeInput}
R> var(pcv$raw.resultS[1, ]) / var(pcv$raw.resultRB[1, ])
\end{CodeInput}
\begin{CodeOutput}
 [1] 23.85134
\end{CodeOutput}
...
And also average simple unbiased estimates of coverage rate for each player can be obtained.
\begin{CodeChunk}
\begin{CodeInput}
R> bcv$coverageS
\end{CodeInput}
...
Due to the fact that the estimation procedures in \pkg{Rgbp} rely on derivatives, the package is extremely quick in fitting the available models. This makes the package ideal to be used in simulation studies where a hierarchical model needs to be fitted at every iteration and where running a full Bayesian model (via MCMC) would be computationally impractical.

\section[acknowledgments]{Acknowledgments}
The authors thank Prof Phil Everson of Swarthmore and the 2012 class of Harvard's Stat 324r: Parametric Statistical Inference and Modeling for their valuable inputs.
