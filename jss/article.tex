...
Also, if we want to check a coverage probability under the different parameter values, $r$ and $\beta$, for example 150 for $r$ and $(-1, 0.5)^{T}$ for $\beta$, the appropriate code is \code{R> coverage(b, A.or.r = 150, reg.coef = c(-1, 0.5), covariates = x, nsim = 10000)}. This adjustment option is helpful for checking coverage probability varying true parameter values. Using this option, we drew Rao-Blackwellized coverage estimates varying only true shrinkage from 0.01 to 0.99.
\begin{CodeChunk}
\begin{CodeInput}
...
R>      ylab = "Rao-Blackwellized Coverage Estimate", lwd = 3, col = 2)
R> abline(h = 0.95)
\end{CodeInput}
\end{CodeChunk}
\begin{figure}[h]
