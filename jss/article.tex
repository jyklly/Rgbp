...
There are two extreme perspectives that the patient could take in this example. The first is that each hospital is identical in their ability to undertake CABG surgeries, in which case the choice of hospitals is irrelevant and the data could be pooled in order to estimate the common death rate for all hospitals (complete pooling). The second, is that the hospitals are totally unrelated in which case the death rate estimate for each hospital would only be a function of the observed data for that particular hospital (no pooling). 


However, in reality, these hospitals are neither completely the same nor different. For example, doctor' ability and experience for dealing with such a surgery vary according to each hospital. At the same time, these hospitals are all subject to the same rules and regulations enforced by the federal and state governments. The hierarchical model provides a happy medium accounting for such a difference and similarity altogether (partial pooling).


The patient can then presume a state-level (NY) hierarchy governing the true death rates ($\lambda$) of CABG surgery for all the hospitals in NY. This perspective is to view the true death rates of those 31 hospitals as sampled from the state-level population distribution. In this example it is assumed that the mean of such state-level death rate distribution is known to be 0.020 ($\lambda_{0}$). 
...
Next, \code{gbp} will help us fit the Poisson hierarchical model with the Gamma conjugate prior distribution on the true death rate $\lambda_{j}$ whose mean is 0.020 ($\lambda_{0}=0.020$) as described in the section \ref{poissonsubsec}. For reference, the number of regression coefficients ($m$) is 0 because we do not need to estimate the prior mean via log-linear regression.
\begin{CodeChunk}
\begin{CodeInput}
...
Intuitively, switching ranks for these two hospitals makes sense. To be specific, their observed death ratios ($y_{j}$, $j=8, 23$) are 0.014 and 0.015 and caseloads ($n_{j}$, $j=8, 23$) are 295 and 602 each. Considering solely the observed death ratios may lead to an unfair comparison because the latter hospital handled more than twice caseloads. \pkg{Rgbp} accounts for this caseload difference, making the estimate of true death rate for the former hospital shrink toward the state-level mean ($\lambda_{0}$=0.02) much harder than that for the latter hospital. This is what happened to the two left-most hospitals in the first plot.
...
This interval plot confirms that the 23rd hospital has the lowest upper bound for the true death rate, not to mention the lowest point estimate. Also it reveals that the 31st hospital may be the second safest because its upper bound of 95\% interval estimate for the true death rate is lower than the 8th hospital's one.
...
Then, how reliable are these intervals? Does our procedure generate interval estimates that have good repeated sampling properties? To answer this question the \code{coverage} function generates pseudo-datasets regarding the estimated $r~(=336.37)$ as a given true value. For reference, we can try any other value of $r$, for example $r=200$, by replacing the code below with `\code{R> pcv <- coverage(p, A.or.r = 200, mean.PriorDist = 0.02, nsim = 1000)}.


Also, \code{gbp} can provide interval estimates with different confidence levels, for example 90\%,  and the appropriate code adjustment is `\code{R> p <- gbp(z, n, Alpha = 0.9, mean.PriorDist = 0.02, model = "poisson")}.'  Then, the below code will evaluate whether interval estimates achieve the 90\% confidence level.
...
It shows that Rao-Blackwellized unbiased estimate is about 24 times more accurate than simple one. Also, as the number of simulation increases, the zig-zagging blue line is expected to be closer to the red line.
...
Then, how much can we trust these interval estimates? The following frequency method checking that regards the estimated values, $\hat{r}$ (=112.95) and $\hat{\beta}~(=(-1.194, ~0.389)^{T})$, as true values will answer it. For reference, if we want to try different true values for example 100 for $r$ and (-1, 0.2) for $(\beta_{0}, \beta_{1})$, the appropriate code will be \code{coverage(b, A.or.r = 100, reg.coef = c(-1, 0.2), nsim = 1000)} instead of below one.
