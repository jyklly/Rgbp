...
The output contains information about observed death ratios ($y_{j}$), caseloads ($n_{j}$), known prior mean ($\lambda_{0}$), shrinkage estimates ($\hat{B}_{j}$), lower bounds (2.5\% percentiles of posterior distributions if 95\% confidence level is given) of interval estimates ($\hat{\lambda}_{j, low}$), posterior means ($\hat{\lambda}_{j}=E(\lambda_{j}\vert \textbf{y})$), upper bounds (97.5\% percentiles of posterior distributions) of interval estimates ($\hat{\lambda}_{j, upp}$), and standard deviations of posterior distributions ($sd(\lambda_{j}\vert \textbf{y})$).
...
In Figure \ref{fig:hospitalcoverage}, the black horizontal line at 0.95 represents the nominal confidence level, the smooth red curve connects Rao-Blackwellized (RB) unbiased coverage estimates for 31 hospitals, and the dashed zagged blue curve passes simple unbiased coverage estimates for these hospitals. As the number of simulations increases, the zig-zagging blue curve would be closer to the smooth red curve by the law of large numbers.



The overall RB unbiased coverage estimate across all the hospitals is 0.955. And none of RB unbiased coverage estimates for 31 hospitals are less than 0.95 regardless of their caseloads ($n_{j}$). This result shows that the interval estimates for this particular dataset accurately achieves 95\% confidence under repeated sampling. Note that these estimates depend on the given true value of $r$, the known prior mean, and the assumption that the model is true.
...
\end{CodeInput}
\begin{CodeOutput}
 [1] 0.0015 0.0015 0.0013 0.0012 0.0012 0.0012 0.0012 0.0012 0.0011 0.0011 0.0011 
[12] 0.0011 0.0010 0.0010 0.0010 0.0010 0.0011 0.0010 0.0010 0.0010 0.0009 0.0010 
[23] 0.0009 0.0009 0.0009 0.0009 0.0008 0.0008 0.0007 0.0006 0.0006
\end{CodeOutput}
\end{CodeChunk}
...
\end{CodeInput}
\begin{CodeOutput}
[1] 0.0063 0.0060 0.0071 0.0063 0.0059 0.0066 0.0063 0.0068 0.0060 0.0076 0.0063 
[12] 0.0059 0.0066 0.0072 0.0070 0.0074 0.0060 0.0062 0.0065 0.0065 0.0075 0.0072 
[23] 0.0061 0.0067 0.0064 0.0065 0.0061 0.0072 0.0067 0.0070 0.0065
\end{CodeOutput}
\end{CodeChunk}

Taking the first hospital as an example, the variance of RB unbiased coverage estimate is about 18 times smaller than that of simple one. It means that 1,000 RB unbiased coverage estimates are as efficient as 18,000 simple unbiased coverage estimates in terms of estimating true coverage probability for the first hospital.

For reference, \code{raw.resultRB} and \code{raw.resultS}, each row of which is about each hospital, in \code{pcv} contain all the individual simulation results.
...
\end{CodeOutput}
\end{CodeChunk}
This output from \code{gbp} summarizes the results. In this Normal-Normal hierarchical model the amount of shrinkage for each unit is governed by the shrinkage factor, $B_i = V_i/(V_i + A)$. As such schools whose variation within the school ($V_{j}$) is less than the between variation ($A$) schools will shrink greater than $50\%$. The results provided by \code{gpb} suggests that there is little evidence that the training provided much added benefit because every school's $95\%$ posterior interval contains 0. 
...
In addition, Rao-Blackwellized (RB) unbiased coverage estimates for each school can be gotten with the command below.
\begin{CodeChunk}
\begin{CodeInput}
R> gcv$coverageRB
\end{CodeInput}
...
All the simulation results (0-1 coverage and RB coverage estimates) are saved in two $8\times1,000$ matrices, \code{gcv$raw.resultRB} and \code{gcv$raw.resultS}, each row of which is about each school.
...
\end{CodeInput}
\begin{CodeOutput}
 [1] 0.0012 0.0013 0.0012 0.0012 0.0013 0.0011 0.0011 0.0009 0.0012 0.0013 
[11] 0.0010 0.0013 0.0010 0.0012 0.0015 0.0010 0.0010 0.0011
\end{CodeOutput}
\end{CodeChunk}

And simple unbiased coverage estimates and their standard errors for each player are
\begin{CodeChunk}
\begin{CodeInput}
R> bcv$coverageS
\end{CodeInput}
...
\end{CodeInput}
\begin{CodeOutput}
 [1] 0.0045 0.0059 0.0066 0.0049 0.0052 0.0044 0.0042 0.0054 0.0056 0.0053 0.0049 
[11] 0.0056 0.0042 0.0049 0.0056 0.0057 0.0052 0.0052
\end{CodeOutput}
\end{CodeChunk}

All the simulation results are saved in two $18\times1,000$ matrices, \code{bcv$raw.resultRB} and \code{bcv$raw.resultS}, each row of which is about each player.
