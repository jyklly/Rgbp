...
\Abstract{
\pkg{Rgbp} is an R package that utilizes Bayesian machinery to provide a method of estimating two-level hierarchical models (also called random-effects mixed models) for Normal, Poisson, and Binomial data in a fast and computationally efficient manner. The method leads to good frequency properties that can be validated via its repeated sampling procedure called ``method checking."  It is found that such Bayesian-frequentist reconciliation allows \pkg{Rgbp} to have attributes desirable both sides of the aisle, working well in small samples and yielding good coverage probabilities for its interval estimates.}
...
The hyper-prior distribution is an assumed distribution on the second-level parameters. With the goal of objectivity in mind \code{gbp} assumes the following non-informative hyper-prior distributions:
\begin{equation}
...
Figure \ref{fig:pseudo} will be helpful to understand this process. As we can see in (\ref{normalpost}), the distribution of each true parameter ($\mu_{j},~j=1,\ldots, k$) is completely determined by two hyper-parameters, $A$ and $\beta_{(m\times1)}$. So, once we fix these hyper-parameters at specific values, we can generate true parameters. Suppose we generated 500 {\boldmath $\mu$}$_{(k\times1)}$'s, \emph{i.e.}, \{{\boldmath $\mu$}$^{(i)}_{(k\times1)},~i=1, \ldots, 500\}$ from the prior distribution in (\ref{normalprior}), where $A$ and $\beta$ are given. Then using (\ref{normalobs}), we can also generate $\{\mathbf{y}^{(i)}_{(k\times1)},~i=1, \ldots, 500\}$ given each {\boldmath$\mu$}$^{(i)}_{(k\times1)}, $ where $i$ indicates the $i$-th simulation. 
...
Let's define an indicator variable, $I^{(i)}_{j}$, which is 1 if the $j$-th group's interval estimate from the $i$-th pseudo-dataset includes $\mu^{(i)}_{j}$ and 0 otherwise. One way to estimate the coverage probability is taking a sample mean of these indicator variables within each group, averaging over all the possible $\mu^{(i)}_{j}$ and $y^{(i)}_{j}$ under fixed $A$ and $\beta$. We call it a simple unbiased estimate of coverage probability. For example, $\frac{1}{500}\sum_{i=1}^{500}I^{(i)}_{1}$ is the simple unbiased estimate of coverage probability for the first group, accounting for possible $\mu^{(i)}_{1}$ and $y^{(i)}_{1}$, $i=1, \ldots, 500,$ given specific $A$ and $\beta$. As the number of pseudo-datasets increases, this simple estimate will converge to the true coverage probability given $A$ and $\beta$.

\subsubsection{Rao-Blackwellized Unbiased Estimates of Coverage Probabilities}
Rao-Blackwellization improves accuracy of an unbiased estimator by taking a conditional expectation given a sufficient statistic. Based on this idea, we define $E(I^{(i)}_{j}\vert y^{(i)}_{j}, A, \beta$), where $A$ and $\beta$ were given at first and $y^{(i)}_{j}$ is a sufficient statistic of the $j$-th group in the $i$-th pseudo-dataset. This expectation is the same as Pr$(\hat{\mu}^{(i)}_{j, low}\le \mu^{(i)}_{j} \le\hat{\mu}^{(i)}_{j, upp}\vert y^{(i)}_{j}, A, \beta)$, where ($\hat{\mu}^{(i)}_{j, low}$, $\hat{\mu}^{(i)}_{j, upp}$) is the $j$-th group's interval estimate on the $i$-th dataset. We can calculate this probability  because we know the distribution of ($\mu^{(i)}_{j} \vert y^{(i)}_{j}, A, \beta$) in (\ref{normalpost}). Note that conditioning on $y^{(i)}_{j}$ is equivalent to conditioning on $\mathbf{y}^{(i)}_{(k\times1)}$ as long as $A$ and $\beta$ are known. Then, we can estimate the first group's coverage probability by $\frac{1}{500}\sum_{i=1}^{500}E(I^{(i)}_{1}\vert y^{(i)}_{1}, A, \beta)$, which is also unbiased but more accurate than the previous simple estimator.
...
\caption{Coverage plot via method checking for 31 hospitals}
...
\caption{Coverage plot via method checking for 18 players}
\label{fig:baseball2}
\end{center}
\end{figure}
...
\pkg{Rgbp} is an R package for estimating and validating a two-level hierarchical model. The package aims to provide a procedure that is not only fast and easy to use but has good frequency properties and can be used in many scenarios. The package provides ``method checking'' functionality to examine repeated sampling properties and test that the method is valid at a wide range of parameter values.
