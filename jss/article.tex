...
\subsection[Known Second-level Mean]{Known Second-level Mean}
What are the true rates, $\lambda_{j},~j=1,\ldots, 31$, governing the death-rates, $z_{j} / n_{j}$, of those 31 hospitals?

We will use 31 hospital dataset that we introduced in 2.2. First of all, can we imagine a bigger population-level hierarchy? In other words, can we consider the true parameters as sampled from a bigger population distribution governing those parameters? In some cases, we might have information about the mean of the second-level. If we think about the 31 hospital dataset, 
As we said, we have data about the medical profiling evaluations for Coronary Artery Bypass Graft (CABG) surgeries of 31 New York hospitals conducted in 1992 (Morris and Lysy, 2012). It comprises of the number of deaths within a month of CABG surgeries in each hospital ($z_{j},~j=1, \ldots, 31$) and total number of patients receiving CABG surgeries (case load) in each hospital ($n_{j},~j=1, \ldots, 31$). The below code is an example of input based on the last ten hospital data.
...
\subsection[Unknown Second-level Mean and No Covariate]{Unknown Second-level Mean and No Covariate}
\begin{figure}[h]
\begin{center}
...
\subsection[Unknown Second-level Mean and One Covariate]{Unknown Prior Mean and One Covariate}
\begin{figure}[h]
\begin{center}
