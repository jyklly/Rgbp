...
The function \code{plot} draws a shrinkage plot and $100\times$\code{Alpha}\% interval plot for random effects with its default argument ``\code{sort = TRUE}'' that draws the $100\times$\code{Alpha}\% interval plot in the ascending order of $n$ for the Binomial and Poisson model and the descending order of standard errors for the Gaussian model. When the argument \code{sort} is set to \code{FALSE}  the $100\times$\code{Alpha}\% interval plot will be plotted in the order of data input.
\begin{CodeChunk}
\begin{CodeInput}
R> plot(b.output)
...
When we fit a model with a known value of the expected random effect, the code \code{coverage} conducts the frequency method checking based on the known value. However, we may sometimes want to conduct the frequency method checking with a different value for the expected random effect. For example, if we want to try a different value of the expected random effect, e.g., 30, we add the argument \code{mean.PriorDist} as follows.
\begin{CodeChunk}
\begin{CodeInput}
R> cov <- coverage(p.output, mean.PriorDist = 30, nsim = 1000)
\end{CodeInput}
\end{CodeChunk}
The frequency method checking will be based on the estimated posterior mode of $r$ and the newly specified value of the expected random effect, 30.

Although the function \code{coverage} does not produce an S3 object, a detailed summary appears in a plot that automatically pops up. In addition, the object \code{cov} that we used to save the result of \code{coverage} as above contains numerical details. We can use the function \code{names} or check the document of \code{coverage} to see the list of numerical details.
\begin{CodeChunk}
\begin{CodeInput}
R> names(cov)
R> ?coverage
...
\end{CodeInput}
\end{CodeChunk}
The ``gbp'' object \code{b.output} contains 2,000 posterior samples of $\alpha$ (\code{b.output$alpha}), a $2,000\times2$ matrix of $\beta$ (\code{b.output$beta}) with each column corresponding to 2,000 posterior samples of each regression coefficient, and a $k\times2,000$ matrix of random effects (\code{b.output$p}) with each row contains posterior samples of each random effect.
