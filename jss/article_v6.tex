...
Although the function \code{coverage} does not produce an S3 object, a detailed summary appears in a plot that automatically pops up. In addition, the variable \code{cov}  where we save  the result of \code{coverage} as above contains numerical details; $k$ RB coverage estimates (\code{cov$coverageRB}) and their standard errors (\code{cov$se.coverageRB}), $k$ simple coverage estimates (\code{cov$coverageS}) and their standard errors (\code{cov$se.coverageS}), overall RB coverage estimate (\code{cov$average.coverageRB}) and its standard error (\code{cov$average.coverageRB}), etc.
...
\end{CodeInput}
\end{CodeChunk}
can be used to load these data into \proglang{R}.
