...
\pkg{Rgbp} can fit a conjugate Binomial-Beta hierarchical model when the group-level aggregate data from $k$ independent  groups are composed of each group's number of successes out of total number of trials. Unlike the Poisson model, which is for the case with a known second-level mean ($m=0$), the expected random effect in the Binomial model is either known ($m=0$) or unknown ($m\ge1$). For $j=1, 2, \ldots, k$,
\begin{align}
...
effects appears if we type the ``gbp'' object at the \proglang{R} console, which
...
The function \code{gbp} can then be used to fit a Poisson-Gamma to the death
