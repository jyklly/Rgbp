...
\Shorttitle{\pkg{Rgbp}: Hierarchical Modeling and Frequency Method Checking on over dispersed data} %% a short title (if necessary)
%% an abstract and keywords
\Abstract{
\pkg{Rgbp} is an \proglang{R} package that utilizes  approximate Bayesian machinery to fit two-level conjugate hierarchical models on overdispersed Gaussian, Poisson, and Binomial data and evaluates whether the resulting approximate Bayesian interval estimates for random effects meet the nominal confidence levels via frequency coverage evaluation. The data that \pkg{Rgbp} assumes comprise observed sufficient statistics for each random effect, such as an average or a proportion, without population-level data.  The approximate Bayesian tool equipped with the adjustment for density maximization produces approximate point and interval estimates for model parameters including second-level variance component, regression coefficients, and random effects. For the Binomial data, the package provides an option to produce  posterior samples of all the model parameters via the acceptance-rejection method. The package provides a quick way to evaluate coverage rates of the resultant Bayesian interval estimates for random effects via a parametric bootstrapping, which we call  ``frequency method checking."  }
...
Gaussian, Poisson, or Binomial data from several independent groups sometimes have more variation than the assumed Gaussian, Poisson, or Binomial distributions on the first-level observed data. To account for the extra-variability, called overdispersion, a two-level hierarchical model regards first-level mean parameters as random effects that come from a population-level conjugate prior distribution. The conjugate prior distribution is exchangeable if no covariates are available and non-exchangeable if the model incorporates covariate information of each group via a linear, log-linear, or logistic regression according to the data type. 


With an assumption of homogeneity within each group, the observed data are sufficient statistics for the random effects, such as averages or proportions, possibly together with each group's covariate information. This type of  data is common for a biological analysis on litter data, a meta analysis, or small area estimation problems.  For these data, the two-level model that \pkg{Rgbp} assumes can be considered as a conjugate hierarchical generalized linear model \citep{hglm2006, hglm2010} without any fixed effects. 


\pkg{Rgbp} takes a Bayesian approach with our special improper hyper-prior distributions on hyper-parameters, the parameters of the conjugate prior distribution. The hyper-prior distributions lead to Stein's harmonic prior known to produce good repeated sampling coverage properties of the Bayesian interval estimates for random effects in a two-level Gaussian hierarchical model \citep{tang2011, morris2012}.  We apply the hyper-prior distributions to Poisson and Binomial hierarchical models. The goal of our hierarchical modeling is to compare random effects among groups by their Bayesian point and interval estimates that have good frequency properties.


When it comes to fitting the model, \pkg{Rgbp} adopts the adjustment for density maximization \citep{carl1988, 1997, tang2011} (ADM), a generalization of a commonly-used restricted maximum likelihood method (REML). The ADM uses Beta distributions to approximate the posterior distributions of shrinkage factors, functions of a second-level variance component, via maximization like a Delta method that uses a Normal distribution to obtain the approximate distribution of a function of a parameter with its MLE and observed information plugged-in.  The ADM is more appropriate than the Delta method (or REML) because the support of each shrinkage factor is between 0 and 1, not ($-\infty,~\infty$). Also, the ADM is free of boundary effects that REML suffers from (e.g. a zero estimate for the second-level variance component). Using the approximate estimates for the posterior mean and variance of the shrinkage factors, \pkg{Rgbp} obtains  approximate posterior mean and variance of the random effect. Finally, \pkg{Rgbp} approximates the posterior distributions of random effects by a Pearson family distribution whose two parameters are matched to the estimated posterior mean and variance of random effects. 

For the Gaussian hierarchical model, \pkg{Rgbp} approximates the posterior distribution of the random effects by a skewed Normal distribution whose three parameters are matched to the first three posterior moment estimates for random effects. For the Binomial  model, \pkg{Rgbp} provides an option to draw independent posterior samples of all the model parameters including second-level variance component, logistic regression coefficients, and random effects,  via an acceptance-rejection method instead of the approximate tool.


In addition to fitting hierarchical models, \pkg{Rgbp} provides a quick way to evaluate the repeated sampling coverage rates of the resulting Bayesian interval estimates for random effects. It is a unique procedure that distinguishes \pkg{Rgbp}  from any other \proglang{R} packages for similar multilevel models, such as \pkg{hglm} for conjugate hierarchical generalized models, \pkg{arm} for Bayesian hierarchical models, and \pkg{hbsae} for hierarchical Bayesian small area estimation. The evaluation procedure which we call ``frequency method checking'' adopts a parametric bootstrapping that generates mock data sets given hyper-parameter values and estimates the coverage rates.



%This package is useful for both frequentist and Bayesian. Bayesians are able to use the package to see results for a non-informative reference prior before and after constructing their own Bayesian hierarchical model and frequentists are provided with a procedure with good repeated sampling properties.
...
%By integrating over $\mu_{j}$ for $j = 1 \ldots k$ a marginal distribution, useful for model checking (Section \ref{discussion}), can be derived
%\begin{equation} \label{normalmarginal}
%y_{j}\vert \boldsymbol{\beta}, A \stackrel{ind}{\sim}\textrm{Normal}(\mu_{0j}, V_{j} + A). 
%\end{equation}
...
%Note that the constant, $1/r$, multiplied to the Gamma distribution below is a scaling factor and the square brackets indicates the [mean, variance] of a distribution. 
\begin{eqnarray}
...
The Binomial-Beta hierarchical model is the last model that \code{gbp} can fit. The notation $y_{j}$ is the number of successes (or failures) out of $n_{j}$ trials of the $j$-th group. Unlike the Poisson-Gamma model, the second-level mean is either known ($m=0$) or unknown ($m\ge1$) a priori.
\begin{eqnarray}
