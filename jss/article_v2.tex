...
Gaussian, Poisson, or Binomial data from several independent groups sometimes have more variation than the assumed Gaussian, Poisson, or Binomial distributions. To account for the extra-variability, called overdispersion, a two-level hierarchical model regards the first-level mean parameters as random effects that come from an exchangeable or non-exchangeable population-level conjugate prior distribution. The observed data are sufficient statistics for the random effects, such as averages or proportions, obtained from several independent groups. The model can incorporate covariate information of each group via a linear, log-linear, or logistic regression according to the data type, which results in a non-exchangeable conjugate prior distribution. 

The package \pkg{Rgbp} fits the two-level models on overdispersed data using the adjustment for density maximization \citep{tang2011}, a generalization of a commonly-used restricted maximum likelihood estimation (REML). 
 and that  The population-level data    of several independent groups are ubiquo
\pkg{Rgbp} is an \proglang{R} package to estimate and validate a two-level model based on  overdispersed  Gaussian, Poisson and Binomial data.  The estimation procedure adopts the adjustment for density maximization \citep{tang2011} for approximate Bayesian point and interval estimates for random effects and the validation involves checking frequency properties of the procedure via repeated sampling (which we call ``frequency method checking"). It is found that even in small samples our procedure yields good frequency properties in comparison to other methods such as Maximum Likelihood Estimation (MLE). 

, producing point and interval estimates of second-level variance component, regression coefficients, and random effects.
