...
\section[Introduction]{Introduction (v3; 27 July 2015)}
...
The following Normal-Normal hierarchical model (or the Gaussian model) assumed by \pkg{Rgbp} is useful when the group-level aggregate data from $k$ independent  groups are continuous variables with known standard errors. The subscript \emph{j} below indicates the \emph{j}th group among \emph{k} groups in the dataset. For $j=1, 2, \ldots, k$, 
\begin{align}
