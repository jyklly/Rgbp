...
\pkg{Rgbp} can estimate a conjugate Poisson-Gamma hierarchical model (or the Poisson model) when the group-level aggregate data from $k$ independent  groups comprise of non-negative count data without upper limit. However, its usage is limited to the case where the expected random effect is known ($m=0$); we may obtain its information from the past studies or from experts. For $j=1, 2, \ldots, k$, 
\begin{align}
